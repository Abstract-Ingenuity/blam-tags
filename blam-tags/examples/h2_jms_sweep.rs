//! Robustness sweep for the H2 render_model → JMS reader. Walks a tag
//! tree, decodes every `.render_model` via the classic path, builds the
//! JMS, and tallies ok / empty / error plus total geometry.
//! Usage: h2_jms_sweep <defs-dir> <tags-root>
...
fn main() {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for e in rd.flatten() {
            ...
            if !p.to_string_lossy().ends_with(".render_model") { continue; }
            let Ok(bytes) = std::fs::read(&p) else { continue };
            if ClassicHeader::parse(&bytes).is_none() { continue; }
            let Ok(layout) = TagLayout::from_json(defs.join("render_model.json")) else { continue };
            let Ok(tag) = read_classic_tag_file(&bytes, layout) else { err += 1; tot += 1; continue };
            tot += 1;
            match JmsFile::from_h2_render_model(&tag) {
    ...
    println!("=== {tot} render_models | {ok} ok | {empty} empty | {err} error ===");
