...
//! its JMS (version 8210). Usage: h2_jms_probe <defs-dir> <tag> <out.jms>
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = header;
    let layout = TagLayout::from_json(PathBuf::from(defs).join("render_model.json"))?;
    let tag = read_classic_tag_file(&bytes, layout)?;

    let jms = JmsFile::from_h2_render_model(&tag)?;
